...
let
  ...
  gradleScript = writeScript "gradle-commands.sh" ''
    ...
    export TERM=dumb
    trap "rm -rf '$GRADLE_USER_HOME'" SIGINT SIGTERM ERR EXIT
    ...
    SRC="$(mktemp -d)"
    trap "rm -rf '$SRC'" SIGINT SIGTERM ERR EXIT
    cp -r "$source/src" "$SRC/"
    chmod -R +w "$SRC/src"
    cd "$SRC/src"
...
writeShellScript "fetch-deps.sh" ''
  ...
  outPath="${
    ...
    else ''$(nix-instantiate --eval -E "dirOf (${code}).meta.position" | tr -d '"')/${depsPath}''
  }"
  ...
  for i in {0..20}; do
    if [ -f "$MITM_CACHE_DIR/out.json" ]; then
      ...
      ${lib.getExe python3} ${./compress-deps-json.py} _tmp_deps.json "$outPath"
      exec rm _tmp_deps.json
    fi
    sleep 1
  done
  exit 1
